...
"""
LOGS 模块 API 测试
自动生成于: 2026-02-04T10:25:51.749932
"""
...
class LogsTest(TestBase):
    ...
    def test_get_2(self):
        ...
        if resource_id:
            actual_path = "/logs/test-id/test-id".replace("test-id", resource_id).replace("{id}", resource_id)
            self.test_endpoint(
                "GET", actual_path,
                "获取日志详情",
                expect_status=200,
            ...
            self.log("⚠️  跳过测试: 获取日志详情 (没有可用的资源 ID)", "WARN")
    ...
    def test_post_3(self):
        ...
        if data and data.get("request_body"):
            ...
            self.log("⚠️  跳过测试: 导出日志为 CSV (缺少测试数据)", "WARN")


    def run_crud_tests(self):
        """运行完整的 CRUD 测试流程"""
        print("\n" + "="*60)
        print(f"Logs 模块 CRUD 测试")
        print("="*60 + "\n")
        ...
        if hasattr(self, 'test_get_1'):
            try:
                ...
                if detail_method:
                    # 替换路径中的 ID
                    resource_id = self.get_resource_id('logs')
                    if resource_id:
                        detail_method()
            except:
                pass
