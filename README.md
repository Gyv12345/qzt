...
**软件源码本身完全免费。** 企智通采用 MIT 协议开源,允许企业商用、私有化部署、二次开发,只需保留源码中的版权声明。**不存在**商业授权费、永久版权费、按用户数或按年订阅的收费项——它也不是 SaaS,系统部署在你自己的服务器上,没有年费。
...
完全支持——私有化部署就是企智通的产品形态(没有 SaaS 在线版):
...
| 系统 | 协议 | 技术栈 | 一句话对比 |
| --- | --- | --- | --- |
| **企智通** | MIT | Go + React | 13 模块一体化、内置 MCP AI、抖音线索直连,中文原生、开箱即用 |
...
选型参考:要「开箱即用 + 中文 + AI 集成 + 获客闭环」选企智通;要生产制造、多语言、海量插件生态选 Odoo/ERPNext;只想拿个权限底座自己写业务选若依系。协议上 MIT(企智通、ERPNext)商用最宽松,GPL/AGPL(Dolibarr、Axelor)对二次分发的开源要求更严。
