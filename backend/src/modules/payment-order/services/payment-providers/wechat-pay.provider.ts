...
export class WechatPayProvider extends BasePaymentProvider {
  ...
  async handleCallback(data: any): Promise<CallbackResult> {
    try {
      ...
      // 验证签名
      const signature =
        data.headers?.["wechatpay-signature"] ||
        data.headers?.["Wechatpay-Signature"];
      const timestamp =
        data.headers?.["wechatpay-timestamp"] ||
        data.headers?.["Wechatpay-Timestamp"];
      const nonce =
        data.headers?.["wechatpay-nonce"] || data.headers?.["Wechatpay-Nonce"];
      const body = data.body;

      if (
        !signature ||
        !this.verifySignature({ timestamp, nonce, body }, signature)
      ) {
  ...
  private decryptData(
    ...
    decipher.setAuthTag(
      Buffer.from(ciphertext.slice(ciphertext.length - 32), "base64"),
    );
