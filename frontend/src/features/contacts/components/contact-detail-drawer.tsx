...
export function ContactDetailDrawer({
  ...
  const fields = useMemo(() => {
    ...
    return [
      ...
      { label: '邮箱', key: 'email', value: contact.email || '', editable: true },
      { label: '职位', key: 'position', value: contact.position || '', editable: true },
      { label: '部门', key: 'department', value: contact.department || '', editable: true },
      { label: '微信号', key: 'wechat', value: contact.wechat || '', editable: true },
      { label: '所属企业', key: 'customerName', value: contact.customerName || '', editable: false },
    ]
  }, [contact])
        ...
        <Tabs defaultValue='detail' className='px-4 pb-6'>
              ...
              {fields.map((field) =>
                field.editable ? (
                  <EditableField
                    ...
                    label={field.label}
                    value={field.value}
                    onSave={(nextValue) => handleInlineSave(field.key as keyof Contact, nextValue)}
                  />
                ) : (
          ...
          <TabsContent value='actions'>
            <div className='space-y-2'>
              ...
              <Button onClick={() => onCreateCustomer(contact)} className='w-full'>
