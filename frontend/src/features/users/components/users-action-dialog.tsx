...
export function UsersActionDialog({
  ...
  const formSchema = z
    .object({
      ...
      email: z.email({
        error: (iss) => (iss.input === '' ? t('user.validation.emailRequired') : undefined),
      }),
    ...
    >
      <DialogContent className='sm:max-w-lg'>
        <DialogHeader className='text-start'>
          <DialogTitle>{isEdit ? t('user.edit') : t('user.addNew')}</DialogTitle>
          <DialogDescription>
            {isEdit ? t('user.editDescription') : t('user.addDescription')} {t('user.clickSave')}
          </DialogDescription>
        </DialogHeader>
          ...
          <Form {...form}>
            ...
            >
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className='grid grid-cols-6 items-center space-y-0 gap-x-4 gap-y-1'>
                    <FormLabel className='col-span-2 text-end'>{t('user.email')}</FormLabel>
                    <FormControl>
                      <Input
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className='grid grid-cols-6 items-center space-y-0 gap-x-4 gap-y-1'>
                    <FormLabel className='col-span-2 text-end'>{t('user.role')}</FormLabel>
                    <SelectDropdown
