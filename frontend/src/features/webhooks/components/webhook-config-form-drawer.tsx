...
import { PLATFORM_OPTIONS } from "../hooks/use-webhooks";
...
interface WebhookConfigFormDrawerProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  editingConfig: WebhookConfig | null;
  onSubmit: (data: WebhookConfigFormValues) => void;
  isSubmitting: boolean;
}

export function WebhookConfigFormDrawer({
  open,
  onOpenChange,
  editingConfig,
  onSubmit,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>
            {editingConfig ? "编辑 Webhook 配置" : "新建 Webhook 配置"}
          </DialogTitle>
          <DialogDescription>
            {editingConfig
              ? "修改现有的 Webhook 配置信息"
