...
行简的安全模型围绕**「你是谁」（认证）**与**「你能做什么」（授权）**两层展开。认证基于 **JWT 双令牌**机制，授权基于 **Casbin RBAC**，并叠加**数据权限范围**控制。本文详细介绍这套机制。
...
行简使用 **JWT（JSON Web Token）** 作为无状态认证凭证。为兼顾**安全**与**用户体验**，采用**双令牌**机制。
...
行简支持**企业微信扫码登录**（OAuth2），可作为账号密码登录的补充：
...
认证解决「你是谁」，授权解决「你能做什么」。行简的授权模型基于 **Casbin** 的 **RBAC**（基于角色的访问控制）。
