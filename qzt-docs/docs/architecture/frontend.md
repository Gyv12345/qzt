...
行简有三套前端，分别面向**后台管理**（admin）、**公众内容**（cms）、**移动办公**（mobile）。它们技术栈各异，但都是后端 RESTful API 的消费方，共享同一套 API 契约与设计语言。本文介绍三套前端各自的架构设计。
...
后台管理系统是行简功能最完整的前端，承载 CRM、审批、HRM、进销存、财务、系统管理等全部模块。它的核心设计目标是：**用最小的代码量完成密集的表单 CRUD**。
