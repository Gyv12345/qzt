...
/**
 * React Bits — DotGrid 交互式点阵背景
 * 来源: https://reactbits.dev/backgrounds/dot-grid (github.com/DavidHDev/react-bits, MIT)
 * 本仓适配: JS → TypeScript(严格模式);其余与上游一致。
 * 依赖: gsap >= 3.13 (InertiaPlugin 自 GSAP 被 Webflow 收购后免费公开)。
 * 鼠标滑过时临近圆点被推动 + 点亮, 点击产生冲击波, 松手弹性归位。
 */
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const dot of dotsRef.current) {
        ...
        let fillStyle: string = baseColor;
        if (dsq <= proxSq) {
