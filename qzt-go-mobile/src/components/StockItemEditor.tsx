...
interface Props {
  ...
  /** in=入库(含单位成本 unit_cost), out=出库(无单位成本) */
  mode?: 'in' | 'out'
}

/** 其他出入库单的商品明细编辑器(多行:选商品 + 数量;入库多单位成本) */
export default function StockItemEditor({ items, onChange, mode = 'in' }: Props) {
  ...
  const pickProduct = (p: { id: number; name: string; price: string }) => {
    if (pickingIndex === -1) {
      onChange([
        ...items,
        { product_id: p.id, product_name: p.name, quantity: '1', unit_cost: mode === 'in' ? String(p.price || '0') : undefined },
      ])
    } else {
      const copy = [...items]
  ...
  const updateRow = (i: number, patch: Partial<StockItem>) => {
    const copy = [...items]
    copy[i] = { ...copy[i], ...patch }
    onChange(copy)
  }
      ...
      {items.map((it, i) => (
        <div key={i} style={{ borderBottom: '1px solid var(--divider)', padding: '8px 0' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 6, alignItems: 'center' }}>
            <Input placeholder="数量" type="number" value={it.quantity} onChange={(v) => updateRow(i, { quantity: v })} style={{ flex: 1, '--font-size': '13px' } as any} />
            {mode === 'in' && (
              <Input placeholder="单位成本" type="number" value={it.unit_cost || ''} onChange={(v) => updateRow(i, { unit_cost: v })} style={{ flex: 1, '--font-size': '13px' } as any} />
            )}
            {mode === 'in' && (
              <span style={{ fontSize: 12, color: 'var(--text-tertiary)', width: 72, textAlign: 'right' }}>
