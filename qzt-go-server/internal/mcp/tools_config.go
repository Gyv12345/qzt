...
func registerSiteConfigTools(s *server.MCPServer) {
	...
	s.AddTool(
		mcp.NewTool("site_config_update",
			...
			mcp.WithString("favicon_url", mcp.Description("网站图标(浏览器标签页)URL")),
			...
			mcp.WithString("geo_region", mcp.Description("SEO地域码(如 CN-henan-luoyang)")),
			mcp.WithString("geo_placename", mcp.Description("SEO地名(如 河南省洛阳市)")),
			mcp.WithString("geo_position", mcp.Description("SEO坐标,纬度;经度(如 34.6197;112.4540)")),
			mcp.WithString("faq_json", mcp.Description("官网FAQ列表JSON([{q,a}]),问答结构供AI搜索引擎引用")),
			mcp.WithString("copyright", mcp.Description("版权声明")),
			mcp.WithString("mcp_url", mcp.Description("MCP服务地址(个人中心API Key页下发)")),
		),
...
func handleSiteConfigUpdate(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	updateReq := &siteUpdateReq{
		...
		GeoRegion:              req.GetString("geo_region", ""),
		GeoPlacename:           req.GetString("geo_placename", ""),
		GeoPosition:            req.GetString("geo_position", ""),
		AnalyticsCode:          req.GetString("analytics_code", ""),
		FaqJSON:                req.GetString("faq_json", ""),
		Copyright:              req.GetString("copyright", ""),
		McpURL:                 req.GetString("mcp_url", ""),
	}
