...
func (s *CalendarService) List(ctx context.Context, userID uint, startDate, endDate string, sources []string) ([]CalendarEvent, error) {
	...
	endDateTime := endDate + " 23:59:59" // datetime 列的当日结束边界
	...
	if want("schedule") {
		...
		if err := db.Table("oa_schedule").Select("id, title, start_time, end_time").
			Where("creator_id = ? AND deleted_at IS NULL AND start_time >= ? AND start_time <= ?", userID, startDate, endDateTime).
	...
	if want("followup") {
		...
		if err := db.Table("follow_up_plan").Select("id, content, plan_time").
			Where("owner_id = ? AND status = 0 AND deleted_at IS NULL AND plan_time >= ? AND plan_time <= ?", userID, startDate, endDateTime).
	...
	if want("opportunity") {
		...
		if err := db.Table("crm_opportunity").Select("id, name, expected_close_date").
			Where("owner_id = ? AND deleted_at IS NULL AND expected_close_date IS NOT NULL AND expected_close_date BETWEEN ? AND ? AND stage NOT IN (?, ?)", userID, startDate, endDate, "WON", "LOST").
	...
	if want("payment") {
		...
		if err := db.Table("crm_contract_payment_plan p").
			...
			Where("p.deleted_at IS NULL AND c.owner_id = ? AND p.plan_date BETWEEN ? AND ?", userID, startDate, endDate).
	...
	if want("meeting") {
		...
		if err := db.Table("oa_meeting_booking").Select("id, title, start_time, end_time").
			Where("organizer_id = ? AND deleted_at IS NULL AND start_time >= ? AND start_time <= ?", userID, startDate, endDateTime).
			Order("start_time ASC").Scan(&rows).Error; err != nil {
	...
	if want("trip") {
		...
		if err := db.Table("oa_business_trip").Select("id, title, start_date, end_date").
			Where("applicant_id = ? AND deleted_at IS NULL AND start_date BETWEEN ? AND ?", userID, startDate, endDate).
	...
	if want("leave") && empID > 0 {
		...
		if err := db.Table("hrm_leave").Select("id, leave_type, start_date, end_date").
			Where("employee_id = ? AND deleted_at IS NULL AND start_date >= ? AND start_date <= ?", empID, startDate, endDateTime).
			Order("start_date ASC").Scan(&rows).Error; err != nil {
	...
	if want("worklog") {
		...
		if err := db.Table("oa_work_log").Select("id, log_date").
			Where("creator_id = ? AND deleted_at IS NULL AND log_date BETWEEN ? AND ?", userID, startDate, endDate).
	...
	if want("project") {
		...
		if err := db.Table("proj_project").Select("id, name, start_date, end_date").
			Where("deleted_at IS NULL AND status NOT IN (?, ?) AND start_date BETWEEN ? AND ? AND (manager_id = ? OR FIND_IN_SET(?, member_ids))", 4, 5, startDate, endDate, userID, userID).
			Order("start_date ASC").Scan(&rows).Error; err != nil {
	...
	if want("task") {
		...
		if err := db.Table("proj_task").Select("id, title, due_date").
			Where("assignee_id = ? AND deleted_at IS NULL AND status IN (?, ?) AND due_date IS NOT NULL AND due_date BETWEEN ? AND ?", userID, 1, 2, startDate, endDate).
	...
	if want("receivable") {
		...
		if err := db.Table("fin_receivable").Select("id, direction, party_name, due_date").
			Where("deleted_at IS NULL AND due_date IS NOT NULL AND due_date BETWEEN ? AND ? AND status IN (?, ?) AND biz_type = ? AND biz_id IN (SELECT id FROM crm_contract WHERE owner_id = ? AND deleted_at IS NULL)", startDate, endDate, 0, 1, "CONTRACT", userID).
			Order("due_date ASC").Scan(&rows).Error; err != nil {
	...
	if want("performance") && empID > 0 {
		...
		if err := db.Table("hrm_performance").Select("id, title, start_date, end_date").
			Where("deleted_at IS NULL AND start_date BETWEEN ? AND ? AND (employee_id = ? OR reviewer_id = ?)", startDate, endDate, empID, userID).
			Order("start_date ASC").Scan(&rows).Error; err != nil {
