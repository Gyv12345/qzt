...
type Claims struct {
	...
	SessionId    string `json:"session_id"`  // refresh id，用于关联信息
	TokenVersion int    `json:"token_ver"`   // 会话版本，与用户表 token_version 比对实现即时失效
	jwt.RegisteredClaims
}

// GenerateAccessToken 创建 access token
func (m *Manager) GenerateAccessToken(userId int32, username, refreshJti string, tokenVersion int) (string, time.Time, error) {
	now := time.Now().UTC()
...
func (m *Manager) GenerateRefreshToken(userId int32, username string, tokenVersion int) (string, string, time.Time, error) {
	now := time.Now().UTC()
...
func (m *Manager) GenerateTokens(userId int32, username string, tokenVersion int) (token *Token, err error) {
	refreshToken, refreshJti, refreshExp, err := m.GenerateRefreshToken(userId, username, tokenVersion)
	if err != nil {
		return nil, fmt.Errorf("generate refresh token error: %w", err)
	}
	accessToken, accessExp, err := m.GenerateAccessToken(userId, username, refreshJti, tokenVersion)
	if err != nil {
...
func (m *Manager) RefreshTokens(refreshToken string) (token *Token, err error) {
	...
	// 生成新的刷新令牌
	newRefreshToken, refreshJti, refreshExp, err := m.GenerateRefreshToken(claims.UserId, claims.Username, claims.TokenVersion)
	if err != nil {
		return nil, fmt.Errorf("generate refresh token error: %w", err)
	}
	// 生成新的访问令牌
	accessToken, accessExp, err := m.GenerateAccessToken(claims.UserId, claims.Username, refreshJti, claims.TokenVersion)
	if err != nil {
		return nil, fmt.Errorf("generate access token error: %w", err)
	}
	return &Token{
