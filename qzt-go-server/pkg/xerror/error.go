...
var (
	...
	HttpTooManyRequests     = NewCode(CategoryHttp, 429, "Too Many Requests")     // 请求过于频繁（限流）
...
var (
	...
	UserDeleteSelfError   = NewCode(CategoryAdminUser, 10205, "不能删除当前登录用户")
	UserNameTelEmptyError = NewCode(CategoryAdminUser, 10206, "用户名或电话不能为空")
	UserNameTelExistError = NewCode(CategoryAdminUser, 10207, "用户名或电话不能重复")
	PwdError              = NewCode(CategoryAdminUser, 10208, "密码须为 6–32 位，并至少包含数字、字母、特殊符号（.!@#$%^&*?_~-）中的两种类型")
	UserListError         = NewCode(CategoryAdminUser, 10209, "用户列表获取失败")
	UserInfoError         = NewCode(CategoryAdminUser, 10210, "用户信息获取失败")
	...
	RoleMenuNotAuthorized      = NewCode(CategoryAdminRole, 10251, "无权分配该菜单权限")
	SuperAdminRoleCannotDelete = NewCode(CategoryAdminRole, 10252, "超级管理员角色不可删除")
)
