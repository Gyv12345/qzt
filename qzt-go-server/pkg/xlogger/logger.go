...
const (
	...
	logTmFmtWithM  = "2006-01-02 15"
	logTmFmtWithH  = "2006-01-02 15"
)

const (
	// FileWriter 仅输出到按天分割的文件。
	FileWriter = "file"
	// MultiWriter 同时输出到控制台和文件。
	MultiWriter = "multi"
	// ConsoleWriter 仅输出到控制台。
...
// Options 日志初始化参数。调用方（app.Init）从配置映射后传入，
// 避免 pkg/xlogger 反向依赖 config 包。
type Options struct {
	// BaseServiceName 服务名，用于控制台日志前缀（如 "qzt-server"）。
	BaseServiceName string
	// BaseServiceVersion 服务版本，用于控制台日志前缀。
	BaseServiceVersion string
	// Output 输出方式：console / file / multi。
	Output string
	// LogEncoder 业务日志编码：console / json。
	LogEncoder string
	// AccessEncoder 访问日志编码：console / json。
	AccessEncoder string
	// LogFileMaxAgeDays 业务日志（debug/info/error）文件保留天数。
	LogFileMaxAgeDays uint32
	// AccessFileMaxAgeDays 访问日志文件保留天数。
	AccessFileMaxAgeDays uint32
}
...
// Init 初始化 Logger 并替换 zap 全局实例。后续业务代码用 zap.L()/zap.S() 或
// 本包提供的 Ctx 变体（InfofCtx/ErrorfCtx 等）打日志。
func Init(basePath string, opt Options) {
	if len(basePath) == 0 {
	...
	logMaxAge := opt.LogFileMaxAgeDays
	accountMaxAge := opt.AccessFileMaxAgeDays

	// 设置日志输出格式
	encoder := getNormalEncoder()
	if opt.LogEncoder == "json" {
		encoder = getJsonEncoder()
	}

	accountEncoder := getAccessNormalEncoder()
	if opt.AccessEncoder == "json" {
	...
	// 根据时间进行分割
	// 统一一个服务名前缀
	svcName := opt.BaseServiceName
	if svcName == "" {
		svcName = "app"
	}
	svcVersion := opt.BaseServiceVersion
	if svcVersion == "" {
		svcVersion = "dev"
	}
	stdoutWithPrefix := zapcore.AddSync(&prefixWriter{
		w:      os.Stdout,
		prefix: xcolor.GreenFont(fmt.Sprintf("[%s:%s] ", svcName, svcVersion)) + xcolor.YellowFont("[logger] | "),
	})
...
func getTimeWriter(filename string, maxAgeDay uint32) zapcore.WriteSyncer {
	...
	if err != nil {
		// Init 期间全局 logger 尚未初始化，降级到 stderr
		_, _ = fmt.Fprintf(os.Stderr, "[logger] create rotate log writer failed for %s, fallback to stdout: %v\n", filename, err)
